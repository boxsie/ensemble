...
const (
	...
	EnsembleService_RegisterService_FullMethodName  = "/ensemble.api.EnsembleService/RegisterService"
)
...
type EnsembleServiceClient interface {
	...
	RegisterService(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ServiceClientMessage, ServiceServerMessage], error)
}
...
func (c *ensembleServiceClient) RegisterService(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[ServiceClientMessage, ServiceServerMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &EnsembleService_ServiceDesc.Streams[2], EnsembleService_RegisterService_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[ServiceClientMessage, ServiceServerMessage]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EnsembleService_RegisterServiceClient = grpc.BidiStreamingClient[ServiceClientMessage, ServiceServerMessage]
...
type EnsembleServiceServer interface {
	...
	// Service registration (multi-service platform)
	RegisterService(grpc.BidiStreamingServer[ServiceClientMessage, ServiceServerMessage]) error
...
func (UnimplementedEnsembleServiceServer) RegisterService(grpc.BidiStreamingServer[ServiceClientMessage, ServiceServerMessage]) error {
	return status.Error(codes.Unimplemented, "method RegisterService not implemented")
}
...
func _EnsembleService_RegisterService_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(EnsembleServiceServer).RegisterService(&grpc.GenericServerStream[ServiceClientMessage, ServiceServerMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type EnsembleService_RegisterServiceServer = grpc.BidiStreamingServer[ServiceClientMessage, ServiceServerMessage]
...
var EnsembleService_ServiceDesc = grpc.ServiceDesc{
	...
	Streams: []grpc.StreamDesc{
		...
		},
		{
			...
			Handler:       _EnsembleService_RegisterService_Handler,
			ServerStreams: true,
			ClientStreams: true,
		},
	},
