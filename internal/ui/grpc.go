...
// GRPCOption configures NewGRPCBackend.
type GRPCOption func(*grpcOpts)
...
// WithTLS enables TLS with system root CAs.
func WithTLS() GRPCOption {
	return func(o *grpcOpts) { o.useTLS = true }
}

// WithTLSInsecureSkipVerify enables TLS but does not verify the server certificate.
// Useful for LAN deployments behind a private CA without distributing the CA cert.
func WithTLSInsecureSkipVerify() GRPCOption {
	return func(o *grpcOpts) { o.useTLS = true; o.tlsInsecure = true }
}
...
// By default the connection is plaintext h2c. Pass WithTLS() or
// WithTLSInsecureSkipVerify() to wrap in TLS (e.g. when going via an HTTPS ingress).
func NewGRPCBackend(target string, privKey ed25519.PrivateKey, options ...GRPCOption) (*GRPCBackend, error) {
	cfg := &grpcOpts{}
...
func (b *GRPCBackend) AddNode(ctx context.Context, onionAddr string) (int, error) {
	resp, err := b.client.AddNode(ctx, &apipb.AddNodeRequest{OnionAddress: onionAddr})
	if err != nil {
		return 0, fmt.Errorf("adding node: %w", err)
	}
	return int(resp.PeersFound), nil
}
