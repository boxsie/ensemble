...
// SetSignaling sets the signaling server and client. The daemon is responsible
// for wiring HandleConnectionRequest into the signaling server's per-service
// unknown-peer callback (the server is multi-tenant and routes per service).
...
// HandleConnectionRequest is invoked by the signaling layer when an unknown
// peer attempts to connect to the node service. It records the pending request
// and emits an event for TUI/gRPC clients to surface to the user.
func (n *Node) HandleConnectionRequest(cr *signaling.ConnectionRequest) {
	n.handleConnectionRequest(cr)
}
