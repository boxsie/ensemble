...
type NodeProvider interface {
	...
	AddNode(ctx context.Context, onionAddr string) (int, error)
}
...
// NewServer creates a gRPC server backed by the given node. The registry and
// chat dispatcher are optional; if either is nil, RegisterService returns
// Unimplemented (the registry case) or surfaces ServiceError on SendMessage
// (the dispatcher case). The daemon wires both once Tor is ready.
func NewServer(n NodeProvider) *Server {
	return &Server{node: n}
}

// SetRegistry attaches the service registry that RegisterService dispatches
// against. May be called once after construction; the daemon does this when
// the registry is created post-Tor-bootstrap.
func (s *Server) SetRegistry(r *services.Registry) {
	s.registry = r
}

// SetChatDispatcher attaches the seam through which a registered service's
// outbound chat is routed. May be called once after construction.
func (s *Server) SetChatDispatcher(d ChatDispatcher) {
	s.chatDispatcher = d
}
...
func (s *Server) AddNode(ctx context.Context, req *apipb.AddNodeRequest) (*apipb.AddNodeResponse, error) {
	peersFound, err := s.node.AddNode(ctx, req.OnionAddress)
	if err != nil {
		return nil, status.Errorf(codes.Internal, "adding node: %v", err)
	}
	return &apipb.AddNodeResponse{PeersFound: int32(peersFound)}, nil
}
